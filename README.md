...
| `data/DepMap/Model.csv` | DepMap 25Q3 cell-line metadata, reduced to the four columns used to select the TNBC cell lines in the explainability scripts. Under the depmap.org terms, see `DATA_LICENSES.md` |
| `data/precision_processed/prism_drug_annotations.csv` | One row per PRISM compound (MOA, target, clinical phase, disease area, indication) from the PRISM Repurposing 19Q4 treatment metadata (CC BY 4.0, curated by the Broad Drug Repurposing Hub), the annotation the candidate selection rule reads |
...
**Data files under `data/` are not covered by that licence.** Each keeps the licence of its source, and `DATA_LICENSES.md` records those licences, the attribution each one requires and the changes made to each file. In short: `prism_drug_annotations.csv` is CC BY 4.0 from the PRISM Repurposing 19Q4 figshare deposit, curated by the Broad Drug Repurposing Hub. `Model.csv` is a four-column subset of DepMap release 25Q3, which has no figshare deposit and is therefore governed by the depmap.org Terms and Conditions rather than by CC BY 4.0, and which does not permit commercial use. `collectri_edges.csv` is a snapshot of CollecTRI as served by OmniPath: a composite resource whose constituent databases carry their own licences (CC BY 4.0 for most, CC BY-SA 4.0 for TRRUST, LGPL-3.0 for HTRI, AFL 3.0 for GEREDB), all of which permit redistribution with attribution. The CollecTRI code repository itself is GPL-3.0.

Results under `results/` and `paper/results/` are per-drug or per-TF summary statistics derived from: DepMap and CCLE expression, release 24Q4, and PRISM 19Q4, both from figshare deposits under CC BY 4.0, plus the cell-line annotations of DepMap 25Q3 under the depmap.org terms; SCAN-B (CC BY 4.0, Mendeley Data `yzxtxn4nmd` v3); TCGA-BRCA (GDC open access); GDSC (`crossval_prism_to_gdsc.csv` only, per-drug correlations, under the GDSC terms of use, which are non-commercial); METABRIC (`survival_METABRIC.csv`, `shared_drugs.csv`, `forest_positive_controls.csv`, `cox_clinical_comparison.csv`, computed from cBioPortal data that has no explicit licence); and AURORA-US (`aurora_tf_activities.csv`, TF activity scores per sample identified by GEO sample id, derived from the public series GSE209998; no clinical or genotype data are included). The trained checkpoints, `cell_line_embeddings.csv` and the Integrated Gradients tables were computed on a graph that contains OmniPath edges, but they hold model weights, embeddings and attribution scores, not the edges themselves, which is why they are redistributed while the graph and the OmniPath cache are not.

If you use this code or these results, please cite the archived version of this repository (metadata in `CITATION.cff`): DOI 10.5281/zenodo.22657695. That DOI always resolves to the latest release, and every release also gets its own version DOI, listed on the Zenodo record, if you need to point at one exact snapshot.
